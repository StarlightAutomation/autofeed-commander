  ...
  <div class="w-full md:w-2/3 ml-auto mr-auto md:px-0 px-3">
    <h1 class="text-2xl mt-5 border-b pb-2">
      ...
      <span class="font-mono">
        ...
        ({{ currentInstance.address }}:{{ currentInstance.port }})
      </span>
    </h1>
...
import Schedule from "~/components/Instance/Schedule";
...
export default {
  ...
  computed: {
    currentInstance () {
      return this.$store.getters['instances/getInstance'](this.$route.params.id);
    },
    api () {
      const instance = this.currentInstance;
      if (!instance) { throw new Error('Invalid instance: ' + this.$route.params.id); }
  ...
  methods: {
    toggleDeviceState (deviceId, setting) {
      this.api.controlDevice(deviceId, setting).then(() => {
    ...
    saveModifiedSchedule (schedule) {
      this.$store.dispatch('api/saveSchedules', {
    ...
    getDeviceStatuses () {
      for (const i in this.deviceStatus) {
        ...
        this.api.getDeviceStatus(device.id).then((res) => {
          this.deviceStatus[i].status = res.status;
        }).catch(error => console.error(error));
      }
    },
  },
